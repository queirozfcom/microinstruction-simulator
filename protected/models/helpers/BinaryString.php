...
/**
 * IMPORTANT: each bit is either "1" or "0" (i.e. they are numeric strings)
 */
...
//                        return trim($output);
    ...
    public function setZero() {
        ...
        if ($numberOfArgumentsPassed === 0)
            throw new MicroinstructionException("TOO FEW ARGUMENTS");
        elseif ($numberOfArgumentsPassed > $this->length)
            throw new MicroinstructionException("TOO MANY ARGUMENTS");
        else {
            $args = func_get_args();
            foreach ($args as $arg) {
                $this->set("zero", $arg);
    ...
    public function setIntValueStartingAt($intVal, $length, $startingIndex) {
        if (($length + $startingIndex) > $this->length)
            throw new BinaryStringException("you're trying to store a number that does not fit into your string");
        if ($intVal > pow(2, $length))
            throw new BinaryStringException(" this number is too big for the given length");
    ...
    public function andWith(BinaryString $bs) {

        $result = new BinaryString($this->length);
        for ($i = 0; $i < $this->length - 1; $i++) {
            if ($this[$i] === "0" or $bs[$i] === "0") {
                $result->setZero($i);
            } else {
                $result->setOne($i);
            }
    ...
    public function orWith(BinaryString $bs) {

        $result = new BinaryString($this->length);
        for ($i = 0; $i < $this->length - 1; $i++) {
            if ($this[$i] === "1" or $bs[$i] === "1") {
                $result->setOne($i);
            } else {
                $result->setZero($i);
            }
    ...
    public function norWith(BinaryString $bs) {
        $result = new BinaryString($this->length);
        for ($i = 0; $i < $this->length; $i++) {
            if ($this[$i] === "1" or $bs[$i] === "1") {
                $result->setZero($i);
    ...
    public function nandWith(BinaryString $bs) {
        $result = new BinaryString($this->length);
        for ($i = 0; $i < $this->length; $i++) {
            if ($this[$i] === "0" or $bs[$i] === "0") {
                $result->setOne($i);
    ...
    public function xorWith(BinaryString $bs) {
        $result = new BinaryString($this->length);
        for ($i = 0; $i < $this->length; $i++) {
            if ($this[$i] === "1" and $bs[$i] === "0") {
                $result->setOne($i);
            } elseif ($this[$i] === "0" and $bs[$i] === "1") {
                $result->setOne($i);
            } else {
                $result->setZero($i);
            }
